...
### External keyboard listeners use the wrong React lifecycle boundary

- Symptom: an immediate Escape can observe the state from the previous render, a portal or focused iframe can bypass a component-local `onKeyDown`, or the first application shortcut after startup can be missed even after changing `useEffect` to `useLayoutEffect`.
- Reproduction path: change a reader state and immediately press Escape from its iframe; repeat with a nested portal and with a global shortcut sent before the deferred React root has committed.
- Root cause: callback freshness, event-document ownership, and listener lifetime are separate boundaries. A passive effect leaves a commit-to-effect stale-closure window; DOM bubbling never crosses iframe documents and may not cross the expected portal ancestry; no React effect can install a listener before the first commit.
- Fix direction: keep a stable external listener proxy and refresh only its callback ref in `useLayoutEffect` when post-commit input must see the new render; subscribe to every window that can own focus for cross-document overlays; install truly pre-commit application shortcuts at bootstrap. Do not mechanically convert effects, force focus, or accumulate parallel listener paths.
- Verification gate: use ordinary keyboard input. Prove the immediate post-state-change iframe event, one-layer Escape ordering from main and frame documents, and a shortcut sent during startup. Check that each event has one owner on every engine.

### WebKit iframe event workaround re-enables standalone document scripts

- Symptom: adding `allow-scripts` to restore parent-installed callbacks in WebKit makes an SVG spine script execute even though ordinary XHTML book scripts remain blocked.
- Reproduction path: render both XHTML and standalone `image/svg+xml` spine items through `Section.render()` and the normal iframe load path; each document attempts a same-origin parent mutation.
- Root cause: WebKit requires `allow-scripts` with `allow-same-origin` for parent-installed frame event callbacks, so script blocking moves to the compensating CSP. A CSP `meta` is enforced only from an HTML `head`; standalone SVG serialization contains neither `<html>` nor `<head>`, and a conditional injector can silently omit the policy.
- Fix direction: retain the WebKit-required sandbox capability pair, inject the blocking CSP before authored executable content, and wrap standalone serialized roots in a minimal HTML document with the CSP in its head. Preserve the explicit scripted-content opt-in.
- Verification gate: browser integration must prove real reader events work and scripts remain blocked for both XHTML and standalone SVG spine shapes in Chromium and WebKit.
...
- Root cause: in vertical writing, CSS Multicol Level 1 `column-width` is the vertical inline size rather than the physical page width. Chromium supports the Multicol Level 2 `column-height` and `column-wrap` controls needed to wrap physical pages horizontally, while WebKit does not; WebKit instead exposes `-webkit-column-axis: horizontal`. That private mode ignores `column-width` and `column-count` for physical fragmentation and derives the physical column size from the pagination root's width. Leaving the root at spread width produces one spread-sized fragment, while writing the horizontal page width back through `-webkit-column-width` during zoom overwrites the correctly scaled vertical inline extent. Applying only the standard Level 2 path makes WebKit overflow vertically, undercount pages, and turn directly into the next section. OPF RTL describes page progression and must not be forced onto the vertical text's inline direction.
- Fix direction: keep the same physical height and four-side padding, then select the column model by CSS capability. Where `column-height` and `column-wrap` are supported, use Multicol Level 2 with `column-width` equal to writable physical height, `column-height` equal to writable single-page width, `column-count: 1`, `column-wrap: wrap`, `column-gap: 0`, and `row-gap` equal to the physical spread gap. Otherwise use WebKit's `-webkit-column-axis: horizontal`, size the effective pagination root to one physical page stride, retain `column-width` as the vertical inline extent, and use the physical spread gap through the legacy gap alias. During zoom, preserve that root as `(single-page content width + gap) / zoom` in both single- and double-page modes, inversely scale the padding, inline column width, and gap, and do not replace `-webkit-column-width` with the horizontal page width. Keep the manager's page progression RTL while leaving rendered text direction author-controlled.
...
- Root cause: the note content container was always declared `overflow: auto`, so WebView scrollbar presentation could appear even when neither physical axis overflowed. The first conditional attempt compared `scrollHeight` with rounded `clientHeight`; in WebView2, glyph ink can extend a few pixels beyond a complete line box, so a source note measured `clientHeight = 72` and `scrollHeight = 75` even though all content fit naturally and the page allowed `675.5px`. WebKit can report `scrollHeight === clientHeight` for the same oversized glyphs because CSSOM View defines the scrolling area from layout-box edges and does not require glyph ink outside a line box to enlarge it. Treating either physical axis as scrollable also lets an unrelated cross-axis difference create the wrong scrollbar. Pairing `overflow-x: hidden` with `overflow-y: visible` is invalid for this purpose because CSS computes the visible axis to `auto` when the other axis is hidden, scroll, or auto.
...
- Verification gate: focused browser interaction coverage must prove text Range bounds exceed the compact line-box/client height without enabling scrolling while `scrollHeight` remains below `maxHeight`, and that content above `maxHeight` still enables vertical scrolling. Do not require glyph ink to change `scrollHeight` identically across engines. Final acceptance must use a real Tauri release client with an affected native EPUB and record client/scroll/max dimensions plus computed overflow; vertical coverage must retain its physical horizontal scroll path and clipped vertical axis.
...
- Symptom: images or styles referenced with a path such as `/media/page.jpg` remain blank even though the resource was unpacked successfully; excessive parent segments such as `../../../../media/page.jpg` can also resolve outside the current book.
...
- Symptom: computed CSS names the EPUB's embedded font, but WebView font loading rejects it and renders fallback glyphs; the font file exists and its URL loads.
- Reproduction path: open an unpacked EPUB whose `META-INF/encryption.xml` applies `http://www.idpf.org/2008/embedding` to a font, then compare `document.fonts.check()` with the package unique identifier and the font's decoded signature.
- Root cause: ZIP extraction preserves IDPF-obfuscated bytes. A browser cannot decode them because the XOR key is the SHA-1 digest of the current package's normalized unique identifier, not a generic font key.
- Fix direction: during unpack normalization, deobfuscate only declared IDPF font resources, verify a supported font signature before writing, and remove handled encryption declarations so exported EPUBs do not label clear fonts as obfuscated. Keep invalid-key resources untouched; do not guess another publication identifier.
- Verification gate: storage coverage must prove successful decoding and repeated-normalization idempotence with synthetic bytes. A real Tauri client must show the valid book's font face loaded, while a book whose font was obfuscated with another publication's key remains rejected and byte-identical.
