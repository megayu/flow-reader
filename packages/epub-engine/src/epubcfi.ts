...
export interface EpubCFIComponent {
  steps: (EpubCFIStep | undefined)[]
  terminal: EpubCFITerminal | null
}
type CfiState = {
  spinePos?: number
  range: boolean
  base: Partial<EpubCFIComponent>
  path: Partial<EpubCFIComponent>
  start: EpubCFIComponent | null
  end: EpubCFIComponent | null
}
type CfiInput = string | Range | Node | EpubCFI
type CfiSteps = EpubCFIComponent['steps']

import { extend, type, findChildren, isNumber } from './utils/core'

function bracketAssertion(value: string) {
...
class EpubCFI {
  declare str: string
  declare base: Partial<EpubCFIComponent>
  declare spinePos: number
  declare range: boolean
  declare path: Partial<EpubCFIComponent>
  declare start: EpubCFIComponent | null
  declare end: EpubCFIComponent | null

  constructor(
    cfiFrom?: CfiInput,
    base?: string | Partial<EpubCFIComponent>,
    ignoreClass?: string,
  ) {
    var type
   ...
   * @private
   */
  checkType(cfi: CfiInput | undefined) {
  ...
  parseComponent(componentStr: string) {
    ...
    component.steps = steps.map(
      function (this: EpubCFI, step: string) {
  ...
  parseStep(stepStr: string) {
    var type: 'element' | 'text', num, index, id
  ...
  getRange(cfiStr: string): [string | undefined, string | undefined] | false {
  ...
  joinSteps(steps: CfiSteps | undefined) {
    if (!steps) {
      return ''
    }
  ...
  segmentString(segment: Partial<EpubCFIComponent>) {
    var segmentString = '/'
   ...
   */
  compare(cfiOne: string | EpubCFI, cfiTwo: string | EpubCFI) {
    var stepsA, stepsB
  ...
  pathTo(node: Node, offset: number | null, ignoreClass?: string) {
  ...
  equalStep(stepA: EpubCFIStep | undefined, stepB: EpubCFIStep | undefined) {
    if (!stepA || !stepB) {
      return false
    }
  ...
  filter(anchor: Node, ignoreClass?: string) {
    var needsIgnoring
    ...
    if (anchor.nodeType === TEXT_NODE) {
      ...
      needsIgnoring = (anchor.parentNode as Element).classList.contains(
        ignoreClass as string,
      )
    } else {
      isText = false
      needsIgnoring = (anchor as Element).classList.contains(
        ignoreClass as string,
      )
    }
  ...
  patchOffset(anchor: Node, offset: number, ignoreClass?: string) {
    if (anchor.nodeType != TEXT_NODE) {
    ...
    if (
      (anchor.parentNode as Element).classList.contains(ignoreClass as string)
    ) {
    ...
    while (curr.previousSibling) {
      if (curr.previousSibling.nodeType === ELEMENT_NODE) {
        ...
        if (
          (curr.previousSibling as Element).classList.contains(
            ignoreClass as string,
          )
        ) {
  ...
  normalizedMap(
    ...
    for (i = 0; i < len; i++) {
      ...
      if (
        ...
        (children[i] as Element).classList.contains(ignoreClass as string)
      ) {
  ...
  filteredPosition(anchor: Node, ignoreClass?: string) {
  ...
  textNodes(container: Node, ignoreClass?: string | null): Node[] {
    return Array.prototype.slice.call(container.childNodes).filter(function (
      ...
      if (node.nodeType === TEXT_NODE) {
        ...
        ignoreClass &&
        (node as Element).classList.contains(ignoreClass as string)
      ) {
        return true
      }
      return false
  ...
  walkToNode(steps: CfiSteps, _doc?: Document, ignoreClass?: string | null) {
    var doc = _doc || document
    var container: Node | null | undefined = doc.documentElement
    var children
    var step
    ...
    for (i = 0; i < len; i++) {
      ...
      if (step!.type === 'element') {
        ...
        if (step!.id) {
          ...
          children = (container as Element).children || findChildren(container!)
  ...
  findNode(steps: CfiSteps, _doc?: Document, ignoreClass?: string | null) {
    var doc = _doc || document
    var container
    var xpath
  ...
  fixMiss(
    ...
    for (let childIndex in map) {
      ...
      if (map[childIndex] === lastStepIndex) {
        child = children[childIndex as unknown as number]!
  ...
  generateChapterComponent(_spineNodeIndex: number, _pos: number, id?: string) {
    var pos = parseInt(_pos as unknown as string),
