...
import type { ChapterSearchOptions } from './utils/chapter-search'

export interface SectionMatch {
  cfi: string
  excerpt?: string
}
export type SectionRequest = (url: string, type?: string) => Promise<unknown>
export interface SpineItem extends PackagingSpineItem {
  hrefAliases?: string[]
  type?: string
  url?: string
  canonical?: string
  resourceAvailable?: boolean
  next?: () => Section | null | undefined
  prev?: () => Section | null | undefined
  cfiBase?: string
}
export interface SectionHooks {
  serialize: Hook<[string, Section]>
  content: Hook<[Document, Section]>
}
...
function isBitmapMediaType(mediaType: string | undefined) {
...
function serializeWithScriptBlockingCsp(
  doc: Document,
  contents: Element,
  mediaType?: string,
) {
  ...
  var head: Element | undefined = doc.getElementsByTagName('head')[0]
...
export function isRenderableSpineMediaType(mediaType: string | undefined) {
  return Boolean(
...
function createBitmapDocument(url: string) {
...
function processingInstructionAttribute(data: string, name: string) {
...
function preserveSvgXmlStylesheets(doc: Document, contents: Element) {
  var anchor = contents.firstChild
  var nodes = doc.childNodes

  for (var i = 0; i < nodes.length; i++) {
    var node = nodes[i] as ProcessingInstruction
    if (
      node.nodeType !== 7 ||
 ...
 */
class Section {
  declare idref: string | null
  declare linear: boolean
  declare properties: string[]
  declare index: number
  declare href: string
  declare hrefAliases: string[]
  declare type: string | undefined
  declare url: string
  declare canonical: string
  declare resourceAvailable: boolean | undefined
  declare next: () => Section | null | undefined
  declare prev: () => Section | null | undefined
  declare cfiBase: string
  declare hooks: SectionHooks
  declare document: Document | undefined
  declare contents: Element | undefined
  declare output: string | undefined
  declare request: SectionRequest | undefined
   ...
   */
  load(_request?: SectionRequest) {
    var request = _request || this.request || Request
    var loading = new defer<Element>()
    ...
    if (this.contents) {
      ...
      request(this.url, requestType(this.type))
        .then(
          function (this: Section, xml: unknown) {
   ...
   */
  base() {
    return replaceBase(this.document!, this as Section & { idref: string })
  }
   ...
   */
  render(_request?: SectionRequest, options: { blockScripts?: boolean } = {}) {
    var rendering = new defer<string>()
    var rendered = rendering.promise

    this.load(_request)
      .then(
        function (this: Section, contents: Element) {
  ...
  findAsync<Result>(
    ...
    options: ChapterSearchOptions<Result> & {
      mapMatch: (match: { cfi: string; excerpt: string }) => Result
    },
  ): Promise<Result[]>
  findAsync(query: string, options?: ChapterSearchOptions<unknown>) {
  ...
  /** Reload serialized inputs on the next render while retaining the active document. */
  invalidateRender() {
    this.contents = undefined!
    this.output = undefined!
  }
