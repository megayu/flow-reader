...
type ResizeListener = ((event: UIEvent) => void) & { cancel: () => void }
...
function ensureHorizontalScrollbarStyle(ownerDocument: Document) {
  if (
    !ownerDocument ||
...
function setHorizontalScrollbarHidden(container: HTMLElement, hidden: boolean) {
  if (!container) {
    return
  }
  ...
  if (hidden) {
    ...
    ;(
      container.style as CSSStyleDeclaration & { msOverflowStyle: string }
...
class Stage {
  declare settings: StageSettings
  declare id: string
  declare container: HTMLDivElement
  declare wrapper: HTMLDivElement | undefined
  declare element: Element
  declare resizeFunc: ResizeListener | undefined
  declare orientationChangeFunc: ((event: Event) => void) | undefined
  declare containerStyles: CSSStyleDeclaration
  declare containerPadding: Padding

  constructor(_options?: StageSettings) {
    this.settings = _options || {}
    this.id = 'flow-epub-container-' + uuid()
  ...
  create(options: StageSettings) {
    ...
    if (overflow) {
      if (
        ...
        ;(container.style as unknown as Record<string, string>)['overflow-y'] =
          overflow
        ;(container.style as unknown as Record<string, string>)['overflow-x'] =
          'hidden'
      } else if (
        axis === 'horizontal' &&
        (overflow === 'auto' || overflow === 'scroll')
  ...
  getElement(_element: string | Element) {
    var element
  ...
  onResize(func: (event: UIEvent) => void) {
  ...
  onOrientationChange(func: (event: Event) => void) {
  ...
  size(width?: number | string | null, height?: number | string | null) {
    ...
    this.containerPadding = {
      left:
        parseFloat(
          (this.containerStyles as unknown as Record<string, string>)[
            'padding-left'
          ]!,
        ) || 0,
      right:
    ...
    let bodyPadding = {
      left:
        parseFloat(
          (bodyStyles as unknown as Record<string, string>)['padding-left']!,
        ) || 0,
      right:
        parseFloat(
          (bodyStyles as unknown as Record<string, string>)['padding-right']!,
        ) || 0,
      top:
        parseFloat(
          (bodyStyles as unknown as Record<string, string>)['padding-top']!,
        ) || 0,
      bottom:
        parseFloat(
          (bodyStyles as unknown as Record<string, string>)['padding-bottom']!,
        ) || 0,
    }
  ...
  overflow(overflow: string) {
    if (this.container) {
      if (
        ...
        ;(this.container.style as unknown as Record<string, string>)[
          'overflow-y'
        ] = overflow
        ;(this.container.style as unknown as Record<string, string>)[
          'overflow-x'
