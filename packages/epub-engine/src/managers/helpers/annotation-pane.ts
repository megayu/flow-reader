...
export type AnnotationRect = Pick<
  DOMRect,
...
type Clipping = { boundary: number; intervals: Interval[] }
type Line<Id> = {
...
type PaneEvent = Event &
  Partial<
    ...
    >
  > & { touches?: TouchList }
// DOM setAttribute stringifies numeric SVG coordinates.
export type NumericSvg<ElementType extends SVGElement = SVGElement> =
  ElementType & { setAttribute(name: string, value: string | number): void }
...
function svgElement<K extends keyof SVGElementTagNameMap>(
  document: Document,
  name: K,
): NumericSvg<SVGElementTagNameMap[K]> {
  return document.createElementNS(SVG_NAMESPACE, name) as NumericSvg<
    SVGElementTagNameMap[K]
  >
}

function plainRect(rect: RectInput) {
...
function textNodeAtBoundary(
  ...
  return node as Text | undefined
}

function textFragments(range: Range) {
  let root = range.commonAncestorContainer
  let document = root.nodeType === 9 ? (root as Document) : root.ownerDocument
  if (!document) return []
  ...
  while (firstIndex < fragments.length) {
    ...
    let leading = fragment.node.data
      .slice(fragment.start, fragment.end)
      .search(/\S/)
    if (leading >= 0) {
...
function blockGeometry(rect: AnnotationRect, vertical: boolean) {
...
function mergeIntervals(intervals: Interval[]) {
  ...
  for (let interval of sorted) {
    ...
    if (!previous || interval[0] > previous[1])
      merged.push(interval.slice() as Interval)
...
function intersectIntervals(first: Interval[], second: Interval[]) {
  let intersections: Interval[] = []
  let i = 0
  let j = 0
...
function mergeAdjacentFragments(rects: AnnotationRect[], vertical: boolean) {
  let sorted = rects.slice().sort((first, second) => {
...
function normalizeLineFragments<Id>(line: Line<Id>, vertical: boolean) {
  let bands = new Map<
    Id,
    { blockStart: number; blockEnd: number; inlineIntervals: Interval[] }
  >()
...
function boundaryIntervals<Id>(
  ...
  return mergeIntervals(
    line.fragments
      .filter(({ geometry }) =>
        side === 'after' ? geometry.end > boundary : geometry.start < boundary,
      )
...
function containsInterval(intervals: Interval[] | undefined, point: number) {
  return intervals?.some(([start, end]) => start <= point && end >= point)
}

export function normalizeAnnotationRects<Id>(
  entries: { mark: Id; rect: RectInput }[],
  ...
  let result = new Map<Id, AnnotationRect[]>()
  for (let line of lines) {
    for (let fragment of line.fragments) {
...
function cloneEvent(event: PaneEvent, view?: LegacyWindow | null) {
...
function sameRects(
  first: AnnotationRect[] | undefined,
...
export class Pane {
  declare target: HTMLElement & { contentDocument?: Document | null }
  ...
  declare eventTarget: (HTMLElement | Document) & {
    defaultView?: LegacyWindow | null
  }
  declare eventHandlers: [string, (event: Event) => void][]
  ...
  constructor(
    ...
    this.eventTarget = (target.contentDocument ||
      target) as typeof this.eventTarget
    this.eventHandlers = PROXIED_EVENTS.map((type) => {
      let handler = (event: Event) => this.dispatch(event)
  ...
  addMark<MarkType extends Mark>(mark: MarkType) {
    this.compactMarks()
    let group = svgElement(this.element!.ownerDocument, 'g')
    let overlapGroup = mark.attributes?.['data-overlap-group'] as
      | string
      | undefined
    let container: NumericSvg | undefined = this.element
    if (overlapGroup) {
  ...
  removeMark(mark: Mark) {
    ...
    let container = element?.parentNode as SVGElement | null | undefined
  ...
  render() {
    ...
    this.element.style.setProperty(
      'top',
      `${target.top - container.top}px`,
      'important',
    )
    this.element.style.setProperty(
      'left',
      `${target.left - container.left}px`,
      'important',
    )
...
export class Mark {
  ...
  measure() {
    if (
      ...
      !/\s/.test(
        (this.range.startContainer as Text).data[this.range.startOffset]!,
      ) &&
      !/\s/.test(
