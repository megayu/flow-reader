...
describe('annotation pane geometry', function () {
  it('separates adjacent lines and columns without flattening overlapping marks', function () {
    ...
    const localOverlap = normalizeAnnotationRects([
      { mark: 'overlap-current', rect: rect(0, 0, 10, 20) },
      { mark: 'overlap-next', rect: rect(0, 15, 10, 20) },
      { mark: 'clear-current', rect: rect(100, 0, 10, 20) },
      { mark: 'clear-next', rect: rect(100, 21, 10, 8) },
    ])

    assert.equal(localOverlap.get('overlap-current')[0].bottom, 17.5)
    assert.equal(localOverlap.get('overlap-next')[0].top, 17.5)
    assert.equal(localOverlap.get('clear-current')[0].bottom, 20)
    assert.equal(localOverlap.get('clear-next')[0].top, 21)
