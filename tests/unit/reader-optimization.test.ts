...
function testNearDocumentStartHandlesDocumentsWithoutBody() {
  ...
  try {
    ;(global as Record<string, unknown>).Node = { TEXT_NODE: 3 }
    ;(global as Record<string, unknown>).NodeFilter = {
      FILTER_ACCEPT: 1,
      FILTER_REJECT: 2,
      FILTER_SKIP: 3,
      SHOW_ELEMENT: 1,
    ...
    ;(global as Record<string, unknown>).Element = class {}
    ...
    ;(global as Record<string, unknown>).Node = originalNode
    ;(global as Record<string, unknown>).NodeFilter = originalNodeFilter
    ;(global as Record<string, unknown>).Element = originalElement
  }
}
