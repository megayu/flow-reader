...
async function openTagSettings(page: Page) {
  ...
  await settings.getByRole('button', { name: msg('tag.title'), exact: true }).click()
  return settings
}
...
test('tag settings keep selection while search changes the visible select-all scope', async ({ page }) => {
  ...
  const search = settings.getByRole('textbox', { name: msg('tag.search') })
  const clearSearch = settings.getByRole('button', { name: msg('tag.search') })
  ...
  const remove = settings.getByRole('button', { name: msg('action.delete'), exact: true })
  const clearOrphans = settings.getByRole('button', { name: msg('settings.tags.clear_orphans'), exact: true })

  await expect(settings.getByRole('textbox', { name: msg('tag.new') })).toBeVisible()
  ...
  await expect(settings.getByText(msg('tag.none'), { exact: true })).toBeVisible()
  ...
  await clearOrphansDialog.getByRole('button', { name: msg('action.cancel'), exact: true }).click()
...
test('tag settings create, edit, delete, batch-delete, and clear orphan tags', async ({ page }) => {
  ...
  await page.goto('/')
  const settings = await openTagSettings(page)
  const newTag = settings.getByRole('textbox', { name: msg('tag.new') })
  ...
  const save = dialog.getByRole('button', { name: msg('action.save'), exact: true })
  ...
  await dialog.getByRole('button', { name: msg('action.cancel'), exact: true }).click()
  await expect(tagOption(settings, 'Focus Tag')).toBeVisible()
  ...
  await dialog.getByRole('button', { name: msg('action.save'), exact: true }).click()
  await expect(tagOption(settings, 'Priority')).toBeVisible()

  await tagOption(settings, 'Priority').click({ button: 'right' })
  await page.getByRole('menuitem', { name: msg('action.delete'), exact: true }).click()
  ...
  await settings.getByRole('button', { name: msg('action.delete'), exact: true }).click()
  await page
...
test('tag settings merge into a selected tag or a new name and explain name conflicts', async ({ page }) => {
  ...
  await confirm.click()
  await expect(page.getByText(msg('tag.error.name_exists'), { exact: true })).toBeVisible()
  await expect(dialog).toBeVisible()
...
test('library tags can be filtered, pinned, edited, batch-applied, renamed, deleted, and persisted', async ({
  ...
  await tagSection.getByRole('button', { name: msg('tag.new') }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await page
    .getByTestId('library-status-filter')
    .getByRole('button', { name: msg('home.library_filter.all') })
    .click()
  await expect(tagSection.getByRole('textbox', { name: msg('tag.new') })).toHaveCount(0)
  await tagSection.getByRole('button', { name: msg('tag.new') }).click()
  await tagSection.getByRole('textbox', { name: msg('tag.new') }).fill('Later')
  await tagSection.getByRole('textbox', { name: msg('tag.new') }).press('Enter')
  await expect(tagChip(page, 'Later')).toBeVisible()
  ...
  await page
    ...
    .getByRole('menuitem', { name: msg('home.library_filter.pin') })
    .click()
  await expect.poll(() => pinnedTags(page)).toEqual(['tag-research'])

  await tagChip(page, 'Research').click()
  await bookCard(page, 'Beta Plain').click({ button: 'right' })
  await page.getByRole('menuitem', { name: msg('book.edit_details') }).click()
  let editDialog = page.getByRole('dialog')
  await expect(editDialog.getByRole('heading', { name: msg('book.edit_details') })).toBeVisible()
  await expect(editDialog.getByRole('textbox', { name: msg('tag.new') })).toHaveCount(0)
  const titleInput = editDialog.getByRole('textbox', { name: msg('book.title') })
  const creatorInput = editDialog.getByRole('textbox', { name: msg('book.author') })
  const saveBookButton = editDialog.getByRole('button', { name: msg('action.save') })
  ...
  await page.keyboard.press('Tab')
  await expect(editDialog.getByRole('button', { name: msg('action.cancel') })).toBeFocused()
  await page.keyboard.press('Tab')
  ...
  await editDialog.getByRole('button', { name: msg('action.cancel') }).click()

  await bookCard(page, 'Beta Plain').click({ button: 'right' })
  await page.getByRole('menuitem', { name: msg('tag.title') }).click()
  editDialog = page.getByRole('dialog')
  await expect(editDialog.getByRole('heading', { name: msg('home.tag_editor.title') })).toBeVisible()
  const saveBookTagsButton = editDialog.getByRole('button', { name: msg('action.save') })
  await expect(saveBookTagsButton).toBeDisabled()
  await editDialog.getByRole('textbox', { name: msg('tag.new') }).fill('Scratch')
  ...
  await page.getByRole('menuitem', { name: msg('tag.title') }).click()
  editDialog = page.getByRole('dialog')
  await editDialog.getByRole('textbox', { name: msg('tag.new') }).fill(' research ')
  await editDialog.getByRole('button', { name: msg('home.edit.add_tag') }).click()
  ...
  await editDialog.getByRole('textbox', { name: msg('tag.new') }).fill('Priority')
  await editDialog.getByRole('button', { name: msg('home.edit.add_tag') }).click()
  await editDialog.getByRole('textbox', { name: msg('tag.new') }).fill('priority')
  await editDialog.getByRole('button', { name: msg('home.edit.add_tag') }).click()
  ...
  await editDialog.getByRole('button', { name: msg('action.save') }).click()
  ...
  await page.getByRole('button', { name: msg('tag.title'), exact: true }).click()
  ...
  const saveBatchTagsButton = batchDialog.getByRole('button', { name: msg('action.save') })
  ...
  await batchDialog.getByRole('textbox', { name: msg('tag.new') }).fill('Batch')
  ...
  await batchDialog.getByRole('button', { name: msg('action.save') }).click()
  await page.getByRole('button', { name: msg('action.cancel'), exact: true }).click()
  ...
  await tagDialog.getByRole('button', { name: msg('action.save') }).click()
  ...
  await page
    .getByTestId('library-tag-context-menu')
    .getByRole('menuitem', { name: msg('action.delete'), exact: true })
    .click()
