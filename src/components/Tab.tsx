...
export function Tab({
  ...
  const tab = (
    <div
      ...
      aria-label={children}
      className={clsx(
        'relative mx-0.5 mt-0.5 mb-0 flex cursor-pointer items-center gap-1 p-2 pr-1 text-base transition-colors outline-none',
        selected
          ...
          : clsx(
              'text-muted-foreground/60 hover:text-foreground rounded-[10px] before:pointer-events-none before:absolute before:inset-x-0 before:top-0.5 before:bottom-1 before:rounded-[10px] before:opacity-0 before:transition-colors hover:before:bg-[var(--flow-bg-control-hover)] hover:before:opacity-100 hover:before:shadow-[inset_0_0_0_1px_var(--flow-tab-border)]',
              showSeparator &&
