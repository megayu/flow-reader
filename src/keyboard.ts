...
export function isEditableKeyboardTarget(target: EventTarget | null) {
  return (
    target instanceof Element && !!target.closest(KEYBOARD_EDITABLE_SELECTOR)
  )
}

export function isKeyboardCaptureTarget(target: EventTarget | null) {
  return (
    target instanceof Element && !!target.closest(KEYBOARD_CAPTURE_SELECTOR)
  )
}

export function hasBlockingKeyboardOverlay(target: EventTarget | null) {
  return eventTargetDocuments(target).some((doc) =>
    doc.querySelector(KEYBOARD_BLOCKING_OVERLAY_SELECTOR),
  )
}
...
export function isGlobalKeyboardShortcutBlocked(e: KeyboardEvent) {
  return (
    ...
    isKeyboardCaptureTarget(e.target) ||
    hasBlockingKeyboardOverlay(e.target)
  )
}
