  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[min(32rem,calc(100vw-2rem))] max-w-none text-base">
        <DialogHeader>
          <DialogTitle>{t('folder_import.title')}</DialogTitle>
        </DialogHeader>

        <section className="min-w-0 space-y-1.5">
          <h3 className="text-muted-foreground leading-none font-medium">{t('folder_import.selected_folder')}</h3>
        ...
        <section className="space-y-1">
          <h3 className="text-muted-foreground leading-none font-medium">{t('folder_import.search_scope')}</h3>
          <CheckRow
            id={`${id}-recursive`}
            label={t('folder_import.include_subfolders')}
            checked={recursive}
        ...
        <section className="flex min-h-8 flex-wrap items-center gap-x-5 gap-y-1">
          <h3 className="text-muted-foreground leading-none font-medium">{t('folder_import.formats')}</h3>
          <CheckRow
            id={`${id}-epub`}
            label="EPUB"
        ...
        <section className="space-y-1">
          <h3 className="text-muted-foreground leading-none font-medium">{t('folder_import.tags')}</h3>
          <div className="grid grid-cols-1 gap-x-4 sm:grid-cols-3">
            <CheckRow
              id={`${id}-tag-root`}
              label={t('folder_import.tag_root')}
              checked={tagRootDirectory}
              onCheckedChange={setTagRootDirectory}
            />
            <CheckRow
              id={`${id}-tag-intermediate`}
              label={t('folder_import.tag_intermediate')}
            ...
            <CheckRow
              id={`${id}-tag-direct`}
              label={t('folder_import.tag_direct')}
